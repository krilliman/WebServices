...
const auctionService = () => {
    ...
    const placeNewBid = async (id, body, res) =>{
        try{
            ...
            const customer = await customerDb.findById(body.customerId);
            if(customer == null){
            ...
            if(body.price != null){
                if(body.price > maxBid && body.price > auction.minimumPrice){
                    auction.auctionWinner = body.customerId;
                    var upsertData = auction.toObject();
                    delete upsertData._id;
                    const updateHighestBidder = await auctionDB.update({_id: id}, upsertData, {multi: false} ,function (err) {
                        if (err) return handleError(err);
                    });
                    if(body.auctionId == null){
                        body.auctionId = id;
                    }
